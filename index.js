...
http.createServer((_, res) => {
    ...
    // ─── Launch Telegram after 8s network warmup ────────────────
    console.log("⏳ Waiting 8s for Render network warmup...");
    setTimeout(() => {
        const tryLaunch = (attempt) => {
            tgBot.launch()
                .then(() => console.log("✅ Telegram bot active."))
                .catch(e => {
                    console.error(`❌ Telegram attempt ${attempt} failed:`, e.message);
                    if (attempt < 5) {
                        console.log(`🔄 Retrying in 10s... (attempt ${attempt + 1}/5)`);
                        setTimeout(() => tryLaunch(attempt + 1), 10000);
                    } else {
                        console.error("❌ Telegram failed after 5 attempts. Check token.");
                    }
                });
        };
        tryLaunch(1);
